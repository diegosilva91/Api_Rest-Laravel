...
class Actions extends Model
{
    ...
    public function Candle($value)
    {
        ...
        $query=\DB::table(\DB::raw('('.$subquery.') AS t'))
            ->select('t.dates', 't.min_value','t.max_value',\DB::raw('FIRST_VALUE(price_min.current_quantity) OVER price_min AS open_value,
                LAST_VALUE(price_max.current_quantity) OVER price_max AS close_value'))
