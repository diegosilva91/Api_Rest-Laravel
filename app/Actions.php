...
class Actions extends Model
{
    ...
    public function Candle($value)
    {
        ...
        $query=\DB::table(\DB::raw('('.$subquery.') AS t'))
            ->select('t.dates', 't.min_value','t.max_value',\DB::raw('FIRST_VALUE(price_min.current_quantity) OVER price_min AS open_value FROM prices WINDOW prices_min AS(ORDER BY price_min.created_at DESC),
                LAST_VALUE(price_max.current_quantity) OVER price_max AS close_value FROM prices WINDOW prices_max AS(ORDER BY price_max.created_at ASC)'))
