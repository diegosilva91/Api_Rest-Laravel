    ...
    public function Candle($value)
    {
        ...
        $query=\DB::table(\DB::raw('('.$subquery.') AS t'))
            ->select(\DB::raw('t.dates, t.min_value,t.max_value,
                FIRST_VALUE(price_min.current_quantity) OVER(ORDER BY price_min.created_at DESC) open_value,
                LAST_VALUE(price_max.current_quantity) OVER(ORDER BY price_min.created_at ASC) close_value
            '))
